...
def get_models_dir():
    ...
    return ensure_dir("models")

def ensure_output_dir():
    """Ensure the output directory exists, create if it doesn't."""
    return ensure_dir("images/output")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="n",
        help="YOLO model size (n, s, m, l, x) or path to custom model.pt file",
    )
    parser.add_argument(
        "--confidence",
        type=float,
    ...
    try:
        # Handle model size shorthand
        if args.model.lower() in ['n', 's', 'm', 'l', 'x']:
            model_size = args.model.lower()
            model_name = f"yolov8{model_size}.pt"
            models_dir = get_models_dir()
            model_path = models_dir / model_name
            print(f"Using YOLOv8 {model_size.upper()} model")
